...
$(function() {
	...
	doSearch = function() {
		...
		if (commonJs.doValidate($("#fmDefault"))) {
			commonJs.doSearch({
				url:"/sys/sysWc/sysWcAct/getList.do",
				data:{},
	...
	openPopup = function(param) {
		...
		if (param.mode == "Detail") {
			url = "/sys/sysWc/sysWcAct/getDetail.do";
			header = com.header.popHeaderDetail;
		} else if (param.mode == "New" || param.mode == "Reply") {
			url = "/sys/sysWc/sysWcAct/getInsert.do";
			header = com.header.popHeaderEdit;
		} else if (param.mode == "Edit") {
			url = "/sys/sysWc/sysWcAct/getUpdate.do";
			header = com.header.popHeaderEdit;
			height = 648;
		}
	...
	doDelete = function() {
		...
		commonJs.doDelete({
			url:"/sys/sysWc/sysWcAct/exeDelete.do",
			onSuccess:doSearch
		});
	};
	...
	exeExport = function(menuObject) {
		...
		commonJs.doExport({
			url:"/sys/sysWc/sysWcAct/exeExport.do",
