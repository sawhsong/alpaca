...
$(function() {
	...
	renderDataGridTable = function(result) {
		...
		if (dataSet.getRowCnt() > 0) {
			for (var i=0; i<dataSet.getRowCnt(); i++) {
				...
				iconAction.setName("icnAction").setUseFor("action").addAttribute("articleId:"+dataSet.getValue(i, "ARTICLE_ID"))
	...
	exeDelete = function() {
		commonJs.ajaxSubmit({
			url:"/zebra/sample/soapwebservice/exeDelete.do",
			dataType:"json",
			formId:"fmDefault",
			success:function(data, textStatus) {
				var result = commonJs.parseAjaxResult(data, textStatus, "json");
	...
	getAttachedFile = function(img) {
		commonJs.ajaxSubmit({
			url:"/zebra/sample/soapwebservice/getAttachedFile.do",
			dataType:"json",
			data:{
