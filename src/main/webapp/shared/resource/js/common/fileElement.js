	...
	$.nony.fileElement = {
		modifyFileSelectObject : function() {
			...
			for (var i=0; i<fileElements.length; i++) {
				if ("Y" == jsconfig.get("customFileObject") || arguments.length != 0) {
					...
					var btnStyledButton = $("<a type='button' id='btnStyledButton"+id+"' class='btn btn-default' style='float:left;margin-left:1px;height:27px;'><i class='fa fa-folder-open fa-lg'></i></a>");
		...
		addFileSelectObject : function(params) {
			...
			if (params.deleteButton) {
				$btnDel = $("<a type='button' id='btnDel"+idAndName+"' class='btn btn-default' style='margin-top:1px;margin-left:0px;height:27px;' title='Delete file element'><i class='fa fa-times fa-lg'></i></a>");
