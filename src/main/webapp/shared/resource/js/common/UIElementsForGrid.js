...
/**
 * Image - For Data Grid only
 */
UiImage = Class.create();
UiImage.prototype = {
	/**
	 * Constructor
	 */
	 ...
	 */
	toHtmlString : function() {
		var str = "", classNamePrefix = "";

		if ($.nony.lowerCase(this.status) == "disabled") {classNamePrefix = "imgDis";}
		else {classNamePrefix = "imgEn";}

		className = ($.nony.isBlank(this.className)) ? classNamePrefix : classNamePrefix+" "+this.className;

		str += "<img src=\""+this.src+"\"";
		if (!$.nony.isEmpty(this.id)) {str += " id=\""+this.id+"\"";}
		if (!$.nony.isEmpty(this.title)) {str += " title=\""+this.title+"\"";}
		if (!$.nony.isEmpty(this.className)) {str += " class=\""+this.className+"\"";}
		if (!$.nony.isEmpty(this.style)) {str += " style=\""+this.style+"\"";}

		if (!$.nony.isEmpty(this.script)) {
			if ($.nony.lowerCase(this.status) != "disabled") {str += " onclick=\""+this.script+"\"";}
		}
		...
		if (!$.nony.isEmpty(this.status)) {str += " "+this.status;}
